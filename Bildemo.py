...
def scrape_cars(url, start_date, end_date, excel_name_entry):
    car_list = []
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            for car_element in car_elements:
                ...
                if mot_info:
                    ...
                    if start_date <= mot_expiry_date <= end_date:
                        ...
                        print(f"Added car: {mot_info['Bilmodell']}")
            ...
            print(f"Number of cars collected: {len(car_list)}")

        else:
            print(f"Error fetching data from page {current_page}")
            break  
    ...
    columns_order = ["URL", "Bilmodell", "Årsmodell", "Mätarställning", "Utgången Besiktning", "Status", "Regnr", "Telefonnummer", "Klar"]
...
def run_scraper():
    try:
        ...
        result = scrape_cars(main_url, start_date, end_date, excel_name_entry)
